...
class CharCNN(nn.Module):
    def __init__(self, features_in, features_out, sequence_length, p_dropout=0.0):
        ...
        sequence_length_out = 34

        self.conv_layers.append(
            nn.Sequential(nn.Conv1d(features_in, 256, kernel_size=7),
                          nn.ReLU(),
        ...
        conv_to_lin_dim = 256 * sequence_length_out
        assert conv_to_lin_dim >= 1024

        self.conv_layers.append(
            nn.Sequential(nn.Conv1d(256, 256, kernel_size=3),
                          nn.ReLU(),
                          nn.AdaptiveMaxPool1d(conv_to_lin_dim)))

        self.lin_layers.append(
            nn.Sequential(nn.Linear(conv_to_lin_dim, 1024),
                          nn.ReLU(),
        ...
        self.lin_layers.append(nn.Linear(1024, features_out))
